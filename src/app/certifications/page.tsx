...
export default function Certifications(){
  ...
  return <><SiteHeader/><main>
    ...
    <section className="shell cert-grid">{data.certifications.map(c=><article className="bento-card" key={c.name}><div className="card-icon-row certification-card-row"><span className="pill">{c.year}</span></div><h2>{c.name}</h2><p>{c.issuer}</p></article>)}</section>
