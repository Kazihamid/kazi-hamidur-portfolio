...
export function exportJson(data: PortfolioData) { download(JSON.stringify(data, null, 2), "kazi-hamidur-portfolio.json", "application/json"); }
export function exportProjectsCsv(data: PortfolioData) {
  const esc=(v:string)=>`"${String(v).replaceAll('"','""')}"`;
  const rows=[["Title","Category","Description","Tools","Featured"],...data.projects.map(p=>[p.title,p.category,p.description,p.tools.join(" | "),String(p.featured)])];
...
export function exportExcelCompatible(data: PortfolioData) {
  const tables = [
    ...
    ["Projects", [["Title","Category","Description"], ...data.projects.map(p=>[p.title,p.category,p.description])]],
